...
## Basics
### Making a list
You can make a list by calling the function make list. You can provide initial values for this list as function parameters.
```lua
...
```lua
local everyItemInListIsBiggerThanThree = list:all( function( item ) return item > 3 end )
local anyItemInListIsEqualToHello = list:any( function( item ) return item == "hello" end )
```
